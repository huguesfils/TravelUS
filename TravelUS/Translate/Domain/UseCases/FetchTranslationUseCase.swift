...
protocol FetchTranslationUseCase {
    func execute(text: String, from sourceLang: Language, to targetLang: Language) async -> Result<String, DomainError>
}
...
struct DefaultFetchTranslationUseCase: FetchTranslationUseCase {
    ...
    func execute(text: String, from sourceLang: Language, to targetLang: Language) async -> Result<String, DomainError> {
        return await repository.translate(text: text, from: sourceLang, to: targetLang).map { $0.toDomain() }
    }
}
